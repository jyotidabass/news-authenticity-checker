...
class NewsAuthenticityChecker:
    ...
    def check_api_status(self):
        ...
        try:
            # For Vercel deployment, always check environment variables
            # since the Config class gets reset on each request
            google_key = os.getenv("GOOGLE_API_KEY", Config.GOOGLE_API_KEY)
            news_key = os.getenv("NEWS_API_KEY", Config.NEWS_API_KEY)
            openai_key = os.getenv("OPENAI_API_KEY", Config.OPENAI_API_KEY)
            pinecone_key = os.getenv("PINECONE_API_KEY", Config.PINECONE_API_KEY)
            pinecone_env = os.getenv("PINECONE_ENVIRONMENT", Config.PINECONE_ENVIRONMENT)
            ...
            Config.PINECONE_ENVIRONMENT = pinecone_env
            
            logger.info(f"Environment check - Google: {'Yes' if google_key else 'No'}, News: {'Yes' if news_key else 'No'}")
...
if __name__ == '__main__':
    ...
    logger.info(f"GOOGLE_API_KEY: {'***' if os.getenv('GOOGLE_API_KEY') else 'Not set'}")
    logger.info(f"NEWS_API_KEY: {'***' if os.getenv('NEWS_API_KEY') else 'Not set'}")
